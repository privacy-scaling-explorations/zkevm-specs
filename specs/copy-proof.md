...
A copy event consists of multiple copy steps, which the first row in the copy event has `q_first` assigned to 1 and the last row has `q_last` assigned to 1.
...
- Boolean check for `q_step`, `q_first`, and `q_last`
- Check `q_first == 0` when `q_step == 0`
- Check `q_last == 0` when `q_step == 1`
...
- The transition constraints for `RwCounter` and `RwcIncreaseLeft` column
    ...
    - when it's not the last row in a copy event (`q_last == 0`), `RwCounter` increases by `rw_diff` and `RwcIncreaseLeft` decrases by `rw_diff`.
    - when it's the last row in a copy event (`q_last == 1`), `RwcIncreaseLeft` is equal to `rw_diff`.
...
- Constrain the transition for `BytesLeft`
    - when it's not the last step (`q_last[1] != 1`), increase by 1 in the next step
