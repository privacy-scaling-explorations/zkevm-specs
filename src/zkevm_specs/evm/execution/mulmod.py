...
def mulmod(instruction: Instruction):
    ...
    # safety check
    assert r.int_value == (a.int_value * b.int_value + (minus_d * n.int_value)) % MOD

    # check (a * b) + ( -d * n ) == r
    a_times_b = instruction.add_words([a, b])
    instruction.mul_add_words(RLC(minus_d), n, a_times_b[0], r)
