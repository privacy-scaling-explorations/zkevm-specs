...
def test_bytecode_unrolling():
    ...
    for byte in range(256):
        if not is_push(byte):
            ...
            rows.append((0, len(rows), True, byte))
    ...
    for n in range(1, 33):
        ...
        rows.append((0, len(rows), True, Opcode.PUSH1 + n - 1))
        for _ in range(n):
            rows.append((0, len(rows), False, data_byte))
    # Set the hash of the complete bytecode in the rows
    hash = linear_combine(keccak256(bytes(bytecode)), r)
    for i in range(len(rows)):
        rows[i] = (hash, rows[i][1], rows[i][2], rows[i][3])
    # Unroll the bytecode
    unrolled = unroll(bytes(bytecode), r)
    ...
    verify(k, [unrolled], r, True)

def test_bytecode_empty():
    bytecodes = [
        unroll(bytes([]), r),
    ]
    verify(k, bytecodes, r, True)

def test_bytecode_full():
    bytecodes = [
        unroll(bytes([7] * 2**k), r),
    ]
    verify(k, bytecodes, r, True)

def test_bytecode_incomplete():
    bytecodes = [
        unroll(bytes([7] * (2**k + 1)), r),
    ]
    verify(k, bytecodes, r, False)

def test_bytecode_multiple():
    bytecodes = [
        unroll(bytes([]), r),
        unroll(bytes([Opcode.PUSH32]), r),
        unroll(bytes([Opcode.PUSH32, Opcode.ADD]), r),
        unroll(bytes([Opcode.ADD, Opcode.PUSH32]), r),
        unroll(bytes([Opcode.ADD, Opcode.PUSH32, Opcode.ADD]), r),
    ]
    verify(k, bytecodes, r, True)

def test_bytecode_invalid_hash_data():
    unrolled = unroll(bytes([8, 2, 3, 8, 9, 7, 128]), r)
    verify(k, [unrolled], r, True)
