...
from zkevm_specs.util import Word, MAX_MEMORY_SIZE
...
Stack = namedtuple(
    ...
    defaults=[100, 64, 0, 2**64 - 1, 0, 2**64 - 1],
)
...
def test_error_gas_uint_overflow_root(
    ...
    tables = Tables(
        ...
        rw_table=set(
            ...
            .stack_read(tx.id, 1012, Word(stack.cd_offset))
            .stack_read(tx.id, 1011, Word(stack.cd_length))
            .stack_read(tx.id, 1014, Word(stack.rd_offset))
            .stack_read(tx.id, 1013, Word(stack.rd_length))
